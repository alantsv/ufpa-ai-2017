...
fim((2,0)).
%%
%% ?- busca(+Inicial, +Vistado, +Resposta).
%% ?- busca((0,0), [(0,0)], Res). 
%%
...
%alantsv's answer
map_length([Xs], [Len-Xs]):- length(Xs, Len).
map_length([X|Xs], [Len-X | Tail]):- length(X, Len),
                                map_length(Xs, Tail).

smallest_answer(Ans):-all_solutions(S), 
                    map_length(S, Res), 
                    sort(1, <, Res, ResOrd), [Ans|_] = ResOrd.
